...
import { copy, assign } from "./utility";
...
const constant = {
  ...
  scenarioTypes: [new Option(1, "Intro"), new Option(-1, "Outro")],
};
...
const app = {
    ...
    cardContainer: new Vue({
      ...
      methods: {
        ...
        deleteCard: function (id) {
          ...
          cards.length <= 1
            ? app.do.show.notification(
                "error",
                "Unable to delete the last card.",
              )
    ...
    popupContentState: new Vue({
      ...
      methods: {
        ...
        updateArrayOfExportOptions: function () {
          ...
          Object.values(this.savedStates).forEach((side) => {
            side.forEach((item) => {
              item.state.name &&
                arr.push(
                  new Option(item.index, item.index + " - " + item.state.name),
                );
            });
          });
        ...
        getStateName: function () {
          ...
          const state =
            sides.index < this.getSideCount() ? sides.left : sides.right;
        ...
        focus: function (e) {
          ...
          const checkForButton = (_) => {
            ...
            return (
              isRenameButton || isSaveButton || isLoadButton || isDeleteButton
            );
          };
          ...
          const focus = (e) => {
            ...
            const setButtons = (_) => {
              $("#rename-state-button").removeClass(
                "popup-content-button-faded",
              );
              $("#delete-state-button").removeClass(
                "popup-content-button-faded",
              );
        ...
        handleSave: function (e) {
          ...
          const showError = (e) => {
            ...
            app.do.show.notification(
              "error",
              "Please select an existing save.",
            );
          };
        ...
        checkRename: function (e) {
          ...
          this.focused && this.getStateName()
            ? this.handleRename(e)
            : showError(e);
        },
        ...
        handleRename: function (e) {
          const doRename = (e) => {
            $('input[name="' + this.focused.target.id + '"]')
              .focus()
              .addClass("saved-state-slot-rename");
        ...
        delete: function () {
          const sides = this.leftOrRight();
          const side =
            this.getStateIndex() < this.getSideCount()
              ? sides.left
              : sides.right;
        ...
        save: function () {
          ...
          const state =
            sides.index < this.getSideCount()
              ...
              : sides.right.state;
          const isEmpty =
            typeof state.name === "undefined" || !state.name.length
              ? true
              : false;
          ...
          const saveAndComplete = (_) => {
            setTimeout((_) => {
              ...
              const conf =
                val === this.lastSaved.name
                  ? true
                  : window.confirm(
                      "Are you sure you want to save over this file?",
                    );
    ...
    popupContentExport: new Vue({
      ...
      methods: {
        ...
        handleExport: function (e) {
          ...
          Object.values(this.exportSelects).forEach((scenario) => {
            Object.values(scenario).forEach((type) => {
              const handleInputType = (_) => {
                let index = parseInt(type.val(), 10);
                index = index ? index : null;

                const getState = (_) => {
                  const count = app.components.popupContentState.getSideCount();
                  const side =
                    index < count ? savedStates.left : savedStates.right;
          ...
          const showError = (_) => {
            app.do.show.notification(
              "error",
              "Select appropriate saved states in the drop down options. ",
            );
          };
        ...
        parseExportItems: function (items) {
          ...
          const handleNullEvent = (event) => {
            return !event.txt && !event.img && !event.pos && !event.ani
              ? null
              : event;
          };
        ...
        referenceExportInputs: function (e) {
          ...
          containers.forEach((container, index) => {
            ...
            typeof this.exportSelects[scenario] === "undefined"
              ? define()
              : set();
          });
        },
  ...
  start: (_) => {
    ...
    Object.values(constant).forEach((val) => {
  ...
  do: {
      ...
      updateRemaining: (_) => {
        ...
        const handleCards = (_) => {
          const updateCards = (i) => {
            ...
            if (!card.left.value.image) {
              card.left.image = prevCard.left.image;
            }
            if (!card.right.value.image) {
              card.right.image = prevCard.right.image;
            }
            if (!card.left.value.position) {
              card.left.position = prevCard.left.position;
            }
            if (!card.right.value.position) {
              card.right.position = prevCard.right.position;
            }
          };
        ...
        cards.length > parseInt(self.focusedCard.index, 10) + 1 &&
          handleCards();
      },
    },
    ...
    //     const overwriteHistory = (_) =>
    //       (self.history.states.length = self.history.index + 1);
    //     self.history.index < self.history.states.length - 1 &&
    //       overwriteHistory();
    ...
    show: {
      ...
      notification: (type, message, onCloseCallback, position) => {
        ...
        new Noty({
          ...
          .on("afterClose", (_) => onCloseCallback())
          .show();
      },
    },
    ...
    inputs: {
      clear: (_) => {
        const clearInputs = (side) =>
          Object.values(side).map((input) => $(input).val(" "));
    ...
    persistence: {
      hideExportPopup: (_) => {
        ...
        if (!exportPopupHidden()) app.do.popup.export.hide();
      },
        ...
        const open = (_) => {
          ...
          const handleInput = (input) =>
            !input
              ...
              : !input.files
              ? alert("This browser doesn't support uploading a file.")
              : !input.files[0]
              ? alert(
                  "There was a problem finding your file. Please select a file to load.'",
                )
              : read();
          handleInput(input);
        };
  ...
  listeners: {
    ...
    inputs: (_) => {
      ...
      const setText = (left, input) => {
        const side = left
          ? self.focusedCard.card.left
          : self.focusedCard.card.right;
        side.text = $(input).val();
      ...
      const addHistory = (_) =>
        self.focusedCard.card && app.do.history.append();
      ...
      const handlePositionInput = (left, input) => {
        const handleChange = (_) => {
          ...
          side.position =
            constant.imagePositionSuperClass + " " + $(input).val();
      ...
      $(".saved-state-text-field").on("focusout", function (e) {
        ...
        $('input[name="' + target + '"]').removeClass(
          "saved-state-slot-rename",
        );
    ...
    cards: (_) => {
      ...
      $("#card-container").on("click", ".card", function (e) {
        ...
        const thumbnail = app.do.thumbnail.prepareForCard(
          self.focusedCard.card,
        );
      ...
      $(".thumbnail-container").on("click", ".thumbnail", function (e) {
        ...
        $(".card").removeClass("focus");
        document
          .getElementById(self.focusedCard.card.id)
          .classList.add("focus");
    ...
    hotkeys: (_) => {
      $(document).on("keydown", function (e) {
        ...
        const handleDelKey = (_) =>
          self.focusedCard.card && app.do.card.delete(self.focusedCard.id);
